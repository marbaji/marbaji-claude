...
Two models, one plan, a bounded argument. **Claude is the builder and orchestrator. Codex is a read-only critic** that can read the repo and the plan but cannot touch a single file. They communicate strictly through `PLAN_FILE` + a Codex session that persists across rounds. The human enters at exactly two points: kickoff and final sign-off.
...
- **Codex's content filter can refuse the review prompt itself, and the refusal looks like a failed run.** A prompt about a
  sandbox, an interpreter, freezing values, or anything security-shaped that is worded with "adversarial", "find what
...
| `LOG_FILE` | the owning project folder, `~/Desktop/Claude Code/10-projects/<yyyy-mm>-<slug>/review-log_<yyyy-mm-dd>-<slug>.md` | Append-only transcript of the argument (every round's critique + what changed). The artifact. |
...
**If APPROVED:** Present to the user — the final `PLAN_FILE`, a 3-bullet summary of what the argument improved, and the round count. Ask: *"Plan survived N rounds of Codex. Implement it now — Codex builds it (`/codex-build`), Claude builds it, or stop here?"* Only on a yes is code written. **No code is written during the loop.** If the user picks Codex, invoke the `codex-build` skill with `SPEC_FILE=<PLAN_FILE>` and the same `LOG_FILE` — roles flip (Codex writes, Claude reviews the diff) and the build rounds append to the same log.
