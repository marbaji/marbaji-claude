...
- **Learning routing.** A lesson that lives only in a session log is a lesson the next session repeats — nothing loads it in time. At close, each lesson is routed to a home future sessions actually read, chosen by one test: **when will this need to fire?** Unprompted before I act → `~/.claude/work-principles.md` (imported by `~/.claude/CLAUDE.md`, so it loads in every project). While editing certain paths → that repo's `.claude/rules/`, opened as a PR before the session ends. While one skill runs → that skill's gotchas file. Looked up deliberately → a vault note. The session log keeps the story; the home keeps the enforcement.
- **Project memory is frozen, and the directory is swept every close.** Auto Memory still captures corrections when they happen, but its scope key is the working *directory*, not the project — so a lesson learned in one directory was invisible in every other, which produced parallel copies of the same rules under different names (2026-08-05 audit: 148 files across 7 silos, one index at 124 lines / ~3.4k tokens per session, the two largest duplicating each other by concept with zero filename overlap). The 2026-07-31 amendment stopped session-end from *enumerating* auto-memory writes and bloat continued anyway — 11 new files in the following 5 days, every one a mid-session correction — so the leak was placement, not frequency. Session-end now treats the memory directory as a **staging area**: anything staged there is surfaced in the same approval batch as the extractions, re-homed per the table above, verified present in its new home, and only then deleted along with its index pointer. Verification failure leaves the file in place and says so, and a slug the harness keeps re-asserting is left alone after two strikes rather than swept forever. A non-empty memory directory at close is unfinished work. A directory accretes by construction; a curated file can only be edited.
...
Since 2026-08-05 the native layer is a **staging area rather than a store**: Auto Memory still captures corrections at the moment they happen, but the always-loaded home for cross-project principles is `~/.claude/work-principles.md` (imported by `~/.claude/CLAUDE.md`), and session-end sweeps the per-directory memory folders empty each close. The reason is structural — Auto Memory is keyed on the working directory, not the project, so the same lesson learned in two directories yields two copies that cannot see each other. See the learning-routing and frozen-project-memory loops above.
