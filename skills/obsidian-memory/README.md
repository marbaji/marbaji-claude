...
- **Learning routing.** A lesson that lives only in a session log is a lesson the next session repeats — nothing loads it in time. At close, each lesson is routed to a home future sessions actually read: repo rules (path-scoped standards, opened as a PR before the session ends) or a skill's gotchas file (task-scoped). Cross-session habits go to auto-memory at correction time, natively and at Claude's discretion — session-end no longer enumerates an auto-memory bucket (2026-07-31 memory-bloat audit: the counted bucket double-forced writes and the always-loaded index had grown to ~4k tokens per session). The session log keeps the story; the home keeps the enforcement.
