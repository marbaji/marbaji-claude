...
**Resolve the doc from the wikilink in `current-focus.md`, not from the slug.** The heading above each entry carries the full path and the work/personal distinction (`### [[Work/$ORG_NAME/Projects/Content/lesson-production/index]]`, `### [[Personal/Projects/figma-to-site/overview|figma-to-site]]`); the sweep's slug carries neither. `--stale-check` returns no `category` field, and its slug extractor stops at the first `/` after `Projects/`, so a nested entry collapses to its parent directory — the three `Content/...` entries all surface as the slug `Content`, and `Work/$ORG_NAME/Projects/Content.md` is a directory, not a file. Reading a path derived from the slug is therefore a miss on exactly the entries that need the synopsis most. You are already opening `current-focus.md` for the status blurb; take the path from the same heading. Add `.md` — wikilinks omit the extension.

**A NESTED entry is a KNOWN TRAP, with or without siblings.** The trigger is the nesting, not the count: the extractor truncates at the first `/` whether one heading matches or five, so a lone `Work/$ORG_NAME/Projects/Datadog RUM/foo` entry collapses exactly like the three `Content/...` ones and hits every failure below. Test it on the heading, not on the candidate list — if the matching heading has a `/` after the captured slug, this paragraph applies. Where the collapse IS many-to-one, it carries one extra obligation: ask about each matching entry separately, by its full path, giving each its own synopsis, since the user is otherwise asked one question about three projects and cannot answer it correctly even in principle. Either way, **list BOTH the collapsed parent slug AND every matching entry's full path in `focus_updates`** — the two are load-bearing for different halves of the run, and either one alone fails.

- **The parent slug alone is a silent no-op.** `_find_entry_block` requires the character after the slug to be `]` or `|`, so a parent slug matches no entry block; of its five call sites, the four move/remove branches take `block is None: continue` (the fifth is `upsert`, which creates rather than skips). The markdown is left untouched while the sidecar entry is dropped, which satisfies the gate and retires the question rather than the project, with nothing in the change report to say so.
- **The full paths alone fail preflight outright.** The staleness gate is literal string membership: it builds `addressed` from the slug strings across all six operations and tests the CANDIDATE's slug against it. The candidate's slug is the collapsed `Content`, which no full path equals, so the run aborts with exit 2 and `stale project 'Content' ... is unaddressed`. Nothing is written.
- **Both together is the working form.** The parent slug satisfies the gate; each full path moves its own block. Verified on a fixture vault: this is the only one of the three shapes that both passes preflight and lands every entry in the change report.
- **Put the parent slug in `snooze[]` whenever ANY matching entry survives** — that is, unless every one of them was retired or completed. Answers will usually diverge, since you asked about each entry separately, and the placement decides whether the group keeps being swept at all. `move_to_complete`, `move_to_retired` and `remove` POP the sidecar key; `snooze` keeps it and preserves `last_worked_on`. Pop it while a sibling is still in Active and the next `--stale-check` re-seeds the parent at today's date, so the survivors' real staleness is erased and they never surface again — their own keys are never swept, because the extractor only ever yields the collapsed parent. Verified on a fixture vault: retire-one-snooze-two with the parent in `move_to_retired` silently drops both survivors from the sweep; the same answers with the parent in `snooze[]` keep them, still 31 days stale. When every entry really was retired or completed, the parent belongs in that operation and the sidecar is left clean.

(The root defect is the extractor's `([^\]|/]+)` capture in `helpers/session_end.py`, which is what collapses the slug in the first place; until that is fixed, this paragraph is the guard.)
