...
**A slug that matches more than one heading is a KNOWN TRAP: never answer it as one project.** The collapse above is many-to-one, so the single candidate `Content` stands for three separate entries with three separate docs. Two things follow. Ask about each matching entry separately, by its full path, giving each its own synopsis — the user is otherwise asked one question about three projects and cannot answer it correctly even in principle. And **list BOTH the collapsed parent slug AND every matching entry's full path in `focus_updates`** — the two are load-bearing for different halves of the run, and either one alone fails.
