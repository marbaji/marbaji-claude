...
| `project` | `str` | Required | A project slug, or the literal string `none` when the artifact isn't tied to one project. `none` is derived from `source` when the path matches `10-projects/<yyyy-mm>-<slug>/` or `20-areas/<slug>/` (the shared `20-areas/artifacts/` dumping ground is excluded and stays `none`). | `memory-and-workspace-system` |
...
| `confidence` | `Literal["active", "ambiguous", "unknown"]` | Optional (default `"active"`) | Which Claude account published it, as read off the ledger's `account_confidence`. Surfaced with a `⚠ confidence: <value>` marker in the session-end confirmation prompt when not `active`, so Mo confirms the account before approving. | `ambiguous` |

**Output:** upserts `Context/artifacts.md` by URL — one row per published artifact, not one row per publish. A new URL gets a new row (`appended`, or `created` when the note itself is new); an entry whose URL already has a row merges into it (`updated`), advancing that row's `date`/`title`/`account`/`confidence`/`project`/`source` to the latest values and appending the session to its `sessions` list (deduplicated) with `republished` incremented; an entry that changes nothing new (same fields, session already recorded) is `skipped (already exists)`. A note still in the pre-upsert seven-column shape (one row per publish, no `confidence` or `republished` columns) is migrated to the nine-column shape in memory on first read, exactly once, printing `migrated Context/artifacts.md: N rows -> M artifacts` to stdout. Content outside the table (frontmatter, the title, a footer a human added) and any row this helper can't parse (wrong cell count for its shape) are carried through unchanged on every write. The agent fills this bucket by reading `~/.claude/state/artifacts.jsonl` for rows newer than the stamp in `~/.claude/state/artifacts.last`; on approval the stamp advances (see [`session-end.md`](session-end.md) Step 3). The ledger is produced outside this helper: the `artifact-source.sh` hook (`PostToolUse` on the `Artifact` tool) appends one JSON line per publish; a failed publish is not ledgered. A missing ledger file means nothing has published this session — the bucket is simply empty, not an error.
...
| `move_to_active` | `list[str]` | Optional (default `[]`) | Backlog promotion: project slugs whose blocks are moved (description intact) to the top of `## Active Projects`. Stamps `last_worked_on` and clears any snooze — a promoted project gets a fresh grace window. | `[promoted-project]` |
| `snooze` | `list[str \| SnoozeOp]` | Optional (default `[]`) | Defer a due project's prompt. Plain slug = default duration (`snooze_days` 14 for active, `backlog_groom_days` 30 for "keep in backlog"); `{slug, days}` = custom ("snooze 3 weeks" → `days: 21`). Writes `snooze_until` + stamps `last_asked_about`, does NOT stamp `last_worked_on` (days-stale keeps accruing) and does not move the entry. Re-snoozing later resets the window (no cap). | `[paused-project, {slug: other-project, days: 21}]` |
...
| `extractions.artifacts[]` | `extractions` | `Context/artifacts.md` — one row per artifact, upserted by URL; note created with the header row if absent. See `ArtifactEntry` above for the upsert/merge/migration behavior. |
