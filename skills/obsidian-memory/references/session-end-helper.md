...
| `move_to_active` | `list[str]` | Optional (default `[]`) | Backlog promotion: project slugs whose blocks are moved (description intact) to the top of `## Active Projects`. Stamps `last_touched` and clears any snooze — a promoted project gets a fresh grace window. | `[promoted-project]` |
...
| `stale_keep` | `list[str]` | Optional (default `[]`) | "Keep" answer for a due candidate. Does NOT stamp `last_touched` (days-stale keeps accruing), but writes `snooze_until = session_date + keep_days` (7) for active projects or `+ backlog_groom_days` (30) for backlog projects, so re-asking is capped at weekly (active) / monthly (backlog). | `[still-relevant-project]` |

Operations are applied in order: removes first, then move-to-complete, then move-to-retired, then move-to-active, then upserts. After the markdown edits, the staleness sidecar `Context/.focus-meta.json` is updated: `upsert` / `move_to_active` stamp `last_touched` (and clear any snooze); `snooze` sets `snooze_until = session_date + 14d`; `stale_keep` sets `snooze_until` per its section's cadence; `move_to_complete` / `move_to_retired` / `remove` drop the entry. Surface due candidates first with `session_end.py --stale-check` (see [`session-end.md`](session-end.md) Step 2b).

**Staleness gate (preflight):** when the run includes `focus_updates`, preflight computes the due candidates for `manifest.date` and fails (exit 2, one problem line per slug) if any candidate is not covered by `remove[]`, `upsert[]`, `move_to_complete[]`, `move_to_retired[]`, `move_to_active[]`, `snooze[]`, or `stale_keep[]`. This makes the Step 2b sweep self-enforcing — a session end cannot silently skip the questions. Cadences: `## Active Projects` candidates surface after `stale_days` (14) untouched; `## Backlog` candidates surface every `backlog_groom_days` (30) for promote/keep/retire grooming.
