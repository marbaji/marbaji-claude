...
**A slug that matches more than one heading is a KNOWN TRAP: never answer it as one project.** The collapse above is many-to-one, so the single candidate `Content` stands for three separate entries with three separate docs. Two things follow. Ask about each matching entry separately, by its full path, giving each its own synopsis — the user is otherwise asked one question about three projects and cannot answer it correctly even in principle. And **do not put the collapsed parent slug in `focus_updates`**: `_find_entry_block` requires the character after the slug to be `]` or `|`, so a parent slug matches no entry block, and all four call sites treat that as `block is None: continue` — a silent no-op. The markdown is left untouched while the sidecar entry is dropped, which satisfies the preflight gate and retires the question rather than the project, with nothing in the change report to say so. Use each entry's own full path as the slug instead. (The root defect is the extractor's `([^\]|/]+)` capture in `helpers/session_end.py`; until that is fixed, this paragraph is the guard.)
